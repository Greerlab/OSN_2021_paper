...
for (i in 1:length(unique(new_df$group))) {
  inputs = unique(new_df$group)[-i]
  a = new_df[new_df$group==inputs[1],"fold_change"]
  b = new_df[new_df$group==inputs[2],"fold_change"]
  print(inputs)
  x = t.test(a,b, alternative = "two.sided")
  print(x)
  print(x$p.value)
...
OSN10 = CreateSeuratObject(OSN10)
OSN10 = NormalizeData(OSN10)
OSN10 = FindVariableFeatures(OSN10, nfeatures = 2000)
OSN10 = ScaleData(OSN10)
OSN10 = RunPCA(OSN10, npcs = 50)
OSN10@meta.data = subdf
OSN10 = RunHarmony(OSN10, group.by.vars = "orig.ident")
...
df$observed = OSN10$OR_identity
df$barcodes = rownames(df)
write.table(df, "OSN_all_50SPCs_2000G.csv", sep = ",")
saveRDS(OSN10,"OSN_all_50SPCs_2000G.rds")
#LR_LOO.ipynb was used to generate leave one out prediction
...
OR_candidates = c("Olfr55","Olfr654","Olfr745","Olfr788","Olfr1110","Olfr1260")
plots = list()
for (i in 1:length(OR_candidates)) {
  plots[[i]] = ggplot(df[df$OR==OR_candidates[i],], aes(x = x, y = y, color = group))+
    background_image(img)+geom_point(size =2)+xlab("A---P")+ylab("V---D")+xlim(c(1.00000,21.29802))+ylim(c(-4584.6427,292.5073))+theme_classic()+NoLegend()+NoAxes()
}
pdf("Figure3e.pdf", width = 20, height = 30)
wrap_plots(plots, ncol = 2)
dev.off()

##### 3f #####
my_data = as.data.frame(cbind(predicted = dfx$predicted,
                              observed = dfx$observed))
...
my_data = as.data.frame(cbind(predicted = dfy$predicted,
                              observed = dfy$observed))
...
pdf("Figure3f.pdf", width = 4, height = 8)
p1 /
p2
dev.off()
